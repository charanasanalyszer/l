...
function es_exportPDFClass(classId) {
  ...
  // Build column list (periods + break slots)
  const cols = es_buildColList(periods);
  const breakColW  = 14;   // narrow break columns (pts)
  const dayColW    = 22;
  const margin     = 8;
  const breakCount = cols.filter(c => c.type === 'break').length;
  const periodCount= cols.filter(c => c.type === 'period').length;
  const periodColW = (pw - margin*2 - dayColW - breakCount * breakColW) / periodCount;
  const rowH = 18;
  ...
  doc.text(`${es_state.school.name||'School'}  —  ${cls.grade}${cls.stream?' '+cls.stream:''}  —  Weekly Timetable`, margin, 12);
  doc.setFontSize(7); doc.setTextColor(136,146,170);
  doc.text(`Generated: ${new Date().toLocaleDateString()}`, margin, 17);
  ...
  for (let i = 0; i < cols.length; i++) {
    ...
    if (col.type === 'day') {
      doc.setFillColor(30,34,48); doc.rect(x0, y, cw, rowH, 'F');
      doc.setTextColor(136,146,170); doc.setFontSize(7);
      doc.text('Day', x0+2, y+11);
    } else if (col.type === 'break') {
      const isLunch = /lunch/i.test(col.breakObj.label);
      doc.setFillColor(isLunch ? 209 : 254, isLunch ? 250 : 243, isLunch ? 229 : 199);
      doc.rect(x0, y, cw, rowH, 'F');
      doc.setTextColor(isLunch ? 6 : 146, isLunch ? 95 : 64, isLunch ? 70 : 14);
      doc.setFontSize(5);
      const lbl = col.breakObj.label.length > 6 ? col.breakObj.label.substring(0,5)+'.' : col.breakObj.label;
      doc.text(lbl,                   x0+1, y+7);
      doc.text(`${col.breakObj.duration}m`, x0+1, y+13);
    } else {
      doc.setFillColor(30,34,48); doc.rect(x0, y, cw, rowH, 'F');
      doc.setTextColor(136,146,170); doc.setFontSize(7);
      doc.text(`P${col.periodIndex+1}`, x0+1, y+7);
      doc.setFontSize(5);
      doc.text(es_getPeriodTime(col.periodIndex), x0+1, y+13);
    }
  }
  y += rowH;
  ...
  for (let d = 0; d < days; d++) {
    ...
    for (let i = 0; i < cols.length; i++) {
      ...
      if (col.type === 'day') {
        doc.setFillColor(21,25,35); doc.rect(x0, ry, cw, rowH, 'F');
        doc.setTextColor(180,195,220); doc.setFontSize(7);
        doc.text(ES_DAY_NAMES[d], x0+2, ry+11);
      } else if (col.type === 'break') {
        const isLunch = /lunch/i.test(col.breakObj.label);
        doc.setFillColor(isLunch ? 209 : 254, isLunch ? 250 : 243, isLunch ? 229 : 199);
        ...
        if (sub) {
          ...
          const sn = sub.name.length > 13 ? sub.name.substring(0,12)+'…' : sub.name;
          doc.text(sn, x0+1, ry+7);
          const tch = slot.teacherId ? es_state.teachers.find(t => t.id === slot.teacherId) : null;
          if (tch) {
            doc.setFontSize(5.5);
            const tn = tch.name.length > 16 ? tch.name.substring(0,15)+'…' : tch.name;
            doc.text(tn, x0+1, ry+13);
          } else {
            doc.setFontSize(5); doc.setTextColor(255,200,150);
            doc.text('⚠ no teacher', x0+1, ry+13);
          }
        } else {
          doc.setFillColor(22,25,34); doc.rect(x0, ry, cw, rowH, 'F');
        }
      }
    }
  }
...
function es_syncFromCharanas() {
  ...
  const newSubjects = chSubjects.map(sub => {
    ...
    return {
      ...
      priority:      existing?.priority || 'core',
...
/* State */
let et_schedule   = [];   // [{date, slotIndex, subjectId, streamSlots:[{streamId,classLabel,teacherId,teacherName,role}]}]
let et_overrides  = {};   // "dayIdx_slot_subjectId_streamId" → teacherId
...
function etGenerate() {
  ...
  let relevantStreams = streams.filter(s => {
    const cls = classes.find(c => c.id === s.classId);
    return cls; /* only streams that have a parent class */
  });
  if (filterClassId) {
    /* could be a classId or streamId */
    relevantStreams = relevantStreams.filter(s => s.id === filterClassId || s.classId === filterClassId);
  }
  /* If no streams defined, fall back to classes */
  const useStreams  = relevantStreams.length > 0;
  const classUnits  = useStreams
    ? relevantStreams.map(s => ({ id: s.id, label: `${classes.find(c=>c.id===s.classId)?.name||''} ${s.name}`, classId: s.classId, streamTeacherId: s.streamTeacherId }))
    : (filterClassId
        ? classes.filter(c => c.id === filterClassId).map(c => ({ id:c.id, label:c.name, classId:c.id, streamTeacherId:null }))
        : classes.map(c => ({ id:c.id, label:c.name, classId:c.id, streamTeacherId:null })));

  /* Build teacher busy-set per day (date string → Set<teacherId>) */
  const teacherBusy = {};
  const markBusy = (dateStr, tid) => {
    if (!tid) return;
    if (!teacherBusy[dateStr]) teacherBusy[dateStr] = new Set();
    teacherBusy[dateStr].add(tid);
  };
  const isBusy = (dateStr, tid) => teacherBusy[dateStr]?.has(tid);

  /* Get free teachers on a day */
  const getFreeTeacher = (dateStr, excludeIds) => {
    return teachers.find(t => !isBusy(dateStr, t.id) && !excludeIds.has(t.id)) || null;
  };
  ...
  while (subQueue.length > 0) {
    ...
    for (let slot = 0; slot < slotsPerDay && subQueue.length > 0; slot++) {
      const subId  = subQueue.shift();
      const sub    = subjects.find(s => s.id === subId) || { id: subId, name: subId };

      /* For each class unit, find the stream teacher for this subject */
      const streamSlots = classUnits.map(unit => {
        /* 1. Try stream-assignment */
        const sa = streamAssignments.find(a => a.streamId === unit.id && a.subjectId === subId && a.teacherId);
        let teacherId   = sa?.teacherId || null;
        let role        = teacherId ? 'stream' : null;

        /* 2. Fall back to subject's default teacher */
        if (!teacherId) {
          const defaultT = subjects.find(s => s.id === subId)?.teacherId;
          if (defaultT) { teacherId = defaultT; role = 'stream'; }
        }

        /* 3. Check collision — if that teacher is already busy this day, find a free one */
        if (teacherId && isBusy(dateStr, teacherId)) {
          const busyTid = teacherId;
          const freeT   = getFreeTeacher(dateStr, new Set([busyTid]));
          if (freeT) { teacherId = freeT.id; role = 'free'; }
          else        { role = 'overflow'; }
        }

        /* 4. No teacher found at all — assign any free teacher */
        if (!teacherId) {
          const freeT = getFreeTeacher(dateStr, new Set());
          if (freeT) { teacherId = freeT.id; role = 'free'; }
        }

        /* Mark teacher busy for this date */
        markBusy(dateStr, teacherId);

        const teacher = teachers.find(t => t.id === teacherId);
        return {
          streamId:    unit.id,
          classLabel:  unit.label,
          teacherId:   teacherId || '',
      ...
      et_schedule.push({ date: dateStr, slotIndex: slot, subjectId: subId, subjectName: sub.name, streamSlots });
    }
    cursor.setDate(cursor.getDate()+1);
  }

  etRender();
  showToast(`✅ Exam timetable generated — ${et_schedule.length} sessions across ${[...new Set(et_schedule.map(s=>s.date))].length} days.`, 'success');
}
...
function etRender() {
  ...
  if (!et_schedule.length) {
    output.innerHTML = `<div class="empty-state" style="padding:2.5rem 1rem;text-align:center;color:var(--muted)">
      ...
      <p>Select an exam and click <strong>Generate Exam Timetable</strong></p>
    </div>`;
  ...
  /* Build column headers from stream labels */
  const streamLabels = et_schedule[0]?.streamSlots.map(ss => ss.classLabel) || [];

  let html = '';
  const roleColors = { stream:'#10b981', free:'#1a6fb5', overflow:'#f59e0b' };
  const roleLabels = { stream:'Stream Teacher', free:'Free Teacher', overflow:'Overflow' };
  ...
  Object.entries(byDate).forEach(([dateStr, slots]) => {
    ...
    html += `<div style="margin-bottom:1.5rem">
      <div style="font-size:.85rem;font-weight:700;color:#1a6fb5;padding:.4rem .75rem;background:rgba(26,111,181,.08);border-left:3px solid #1a6fb5;border-radius:0 6px 6px 0;margin-bottom:.5rem">
        📅 ${dayLabel}
      </div>
      <div style="overflow-x:auto">
      <table style="width:100%;border-collapse:collapse;font-size:.78rem">
        <thead>
          <tr style="background:var(--bg3,#f1f5f9)">
            <th style="padding:.45rem .65rem;text-align:left;border:1px solid var(--border,#e2e8f0);min-width:90px">Slot</th>
            <th style="padding:.45rem .65rem;text-align:left;border:1px solid var(--border,#e2e8f0);min-width:130px">Subject</th>
            ${streamLabels.map(l => `<th style="padding:.45rem .65rem;text-align:left;border:1px solid var(--border,#e2e8f0);min-width:150px">${l}</th>`).join('')}
          </tr>
        </thead>
        <tbody>`;
    ...
    slots.forEach((slot, si) => {
      html += `<tr style="border-bottom:1px solid var(--border,#e2e8f0)">
        <td style="padding:.45rem .65rem;border:1px solid var(--border,#e2e8f0);color:var(--muted);font-weight:600">
          Slot ${slot.slotIndex + 1}
        </td>
        <td style="padding:.45rem .65rem;border:1px solid var(--border,#e2e8f0);font-weight:700">
          ${slot.subjectName}
        </td>`;
      slot.streamSlots.forEach(ss => {
        const overrideKey = `${dateStr}_${slot.slotIndex}_${slot.subjectId}_${ss.streamId}`;
        const overrideTid  = et_overrides[overrideKey];
        const displayTName = overrideTid ? (teachers.find(t=>t.id===overrideTid)?.name || ss.teacherName) : ss.teacherName;
        const displayRole  = overrideTid ? 'free' : ss.role;
        const dotColor     = roleColors[displayRole] || '#94a3b8';
        html += `<td style="padding:.45rem .65rem;border:1px solid var(--border,#e2e8f0)">
          <div style="display:flex;align-items:center;gap:.4rem">
            <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${dotColor};flex-shrink:0"></span>
            <span style="flex:1">${displayTName}</span>
            <button onclick="etEditSlot('${overrideKey}','${slot.subjectName}','${ss.teacherId || ''}')"
              style="background:none;border:none;cursor:pointer;font-size:.7rem;color:var(--muted);padding:0 2px"
              title="Change invigilator">✏️</button>
          </div>
          <div style="font-size:.65rem;color:${dotColor};margin-left:1.15rem">${roleLabels[displayRole]||''}</div>
        </td>`;
      });
      html += `</tr>`;
    });
...
function etExportPDF() {
  ...
  const { jsPDF } = window.jspdf || {};
  if (!jsPDF) { showToast('PDF library not loaded yet.', 'warning'); return; }
  const doc   = new jsPDF({ orientation:'landscape', unit:'mm', format:'a4' });
  const exam  = exams.find(e => e.id === document.getElementById('etExamSelect').value);
  const title = `Exam Timetable — ${exam?.name || ''}`;
  ...
  const streamLabels = et_schedule[0]?.streamSlots.map(ss => ss.classLabel) || [];
  const head = [['Date','Slot','Subject',...streamLabels]];
  const body = et_schedule.map(s => {
    const d = new Date(s.date+'T00:00:00');
    const dayN = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][d.getDay()];
    const dateLabel = `${dayN} ${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`;
    return [
      dateLabel,
      `Slot ${s.slotIndex+1}`,
      s.subjectName,
      ...s.streamSlots.map(ss => {
        const key = `${s.date}_${s.slotIndex}_${s.subjectId}_${ss.streamId}`;
        const ov  = et_overrides[key];
        return ov ? (teachers.find(t=>t.id===ov)?.name||ss.teacherName) : ss.teacherName;
      })
    ];
  });
  doc.autoTable({ head, body, startY:24, styles:{fontSize:7}, headStyles:{fillColor:[26,111,181]}, alternateRowStyles:{fillColor:[240,247,255]} });
...
function etExportExcel() {
  ...
  if (!XLSX) { showToast('Excel library not loaded yet.', 'warning'); return; }
  const exam   = exams.find(e => e.id === document.getElementById('etExamSelect').value);
  const labels = et_schedule[0]?.streamSlots.map(ss => ss.classLabel) || [];
  const rows   = [['Date','Day','Slot','Subject',...labels]];
  et_schedule.forEach(s => {
    const d = new Date(s.date+'T00:00:00');
    rows.push([
      s.date,
